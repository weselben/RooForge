...
- Forge orchestrator invokes it automatically at session start: `forge-flow` runs first, then `wayfinder` is auto-loaded in chart mode (if no map exists) or work-through-map mode (if a map is provided or exists in the tracker).

## How it works

### Chart the map (first session for a new effort)

1. **Name the destination** — run `grilling` + `domain-modeling` to pin down what this map is finding its way to (a spec, decision, or change). The destination fixes scope. (SKILL.md:84–86)
...
5. **Fire research subagents** — for each `research` ticket, spin up a subagent running `deep-research` in parallel, capturing findings on a throwaway `research/<name>` branch with a context pointer from the ticket. (SKILL.md:92)
...
3. **Resolve it** — zoom as needed: fetch related/closed ticket bodies on demand; invoke skills named in the map's `## Notes`. Default to `grilling` + `domain-modeling`. (SKILL.md:98)
...
- **Map**: load existing map or invoke wayfinder chart mode (grilling + domain-modeling → map + tickets). Forge may dispatch parallel agents for independent fog patches via `dispatching-parallel-agents`. (forge/SKILL.md:33–38)
- **Resolve**: invoke the skill named by the ticket's `wayfinder:<type>` label (`grilling`, `prototype`, `deep-research`, `domain-modeling`, `task`). After every grilling ticket closes, invoke `domain-modeling` to sweep for new terms (update `docs/dev/CONTEXT.md`) and decisions (add ADR to `docs/adr/`). (forge/SKILL.md:40–44)
...
- **grilling** — Default HITL ticket type; always invoked with `domain-modeling` for grilling tickets.
...
- **prototype** — Resolves `prototype` tickets by making a cheap concrete artifact to react to.
