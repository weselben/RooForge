...
- Forge step 7: after `Skill(skill='verification-before-completion')` passes, `Skill(skill='pr-review')` runs in PR mode
- Forge/issue Phase-5-style reviews in local mode for non-GitHub branches

## How it works

1. **WORKTREE** — `Skill(skill='using-git-worktrees')` Step 0. If already isolated on PR head, stay; otherwise `gh pr checkout <n>` (metadata only) and create `.worktrees/pr-<n>-review/` on a branch tracking PR head (SKILL.md:18-19).
...
3. **REVIEW** — `scripts/review-loop.sh <pr-ref> <worktree> [max_iter=5]`. Renders `scripts/templates/review-loop.md` via `../loops/scripts/cavemanize.sh`, drives `../loops/scripts/run_loop.sh`. Reviewer loads `Skill(skill='caveman-review')` + `Skill(skill='verification-before-completion')` + `Skill(skill='using-git-worktrees')`; writes findings to `${TMPDIR:-/tmp}/pr-review-<n>.md` — scratch, never committed (SKILL.md:20-21, `scripts/review-loop.sh:1-38`, `scripts/templates/review-loop.md:1-22`).
...
5. **HANDOFF** — Report review URL, counts by severity, next step (`Skill(skill='pr-resolve')` for 🔴/🟡). Clean up review worktree via `Skill(skill='using-git-worktrees')` (SKILL.md:25-26).
...
- `scripts/review-loop.sh` — Drives `kimi -p` loop via `Skill(skill='loops')`; renders template, runs `run_loop.sh`; supports PR mode (`owner/repo#n`) and local mode (`branch/slug`)
...
- `Skill(skill='forge')` — Orchestrator step 7 runs `Skill(skill='pr-review')` in PR mode; step 8 runs `Skill(skill='pr-resolve')` to consume findings
- `Skill(skill='pr-resolve')` — Consumes `Skill(skill='pr-review')` output; one resolver per finding group in its own worktree off PR head
- `Skill(skill='caveman-review')` — Finding format: `<file>:L<line>: 🔴|🟡|🔵|❓ <problem>. <fix>.`; all posted text goes through `Skill(skill='ste100')`
- `Skill(skill='verification-before-completion')` — Loaded by reviewer; verifies claimed state against actual output
- `Skill(skill='using-git-worktrees')` — Mandatory worktree isolation; reviewer never leaves the worktree
- `Skill(skill='loops')` — Single home for all `kimi -p` iteration (`cavemanize.sh`, `run_loop.sh`); used by `Skill(skill='pr-review')` and `Skill(skill='pr-resolve')`
- `Skill(skill='ste100')` — Public GitHub text formatting; applied to all findings in posted review
...
- `Skill(skill='ste100')` skill is referenced for public GitHub text but not explicitly loaded in templates — relies on implicit convention