...
1. **Create or smart-append AGENTS.md** (SKILL.md:19–36)
   ...
   - If `AGENTS.md` absent at repo root: write template verbatim; load `Skill(skill='use-git-identity')` to verify ownership
...
2. **Grilling session for repo-specifics** (SKILL.md:38–54)
   - Load `Skill(skill='grilling')` skill; interview the user about the repo
...
- `Skill(skill='grilling')` — Used in step 2 to interview the user and capture repo-specific contract
- `Skill(skill='use-git-identity')` — Loaded to verify repo ownership before writing AGENTS.md
- `Skill(skill='forge-flow')` — Runs after forge-init; creates feat branch from main and sets harness goal
- `Skill(skill='forge')` — The orchestrator that forge-init prepares the repo for; loads automatically on session start
- `Skill(skill='wayfinder')` — Referenced in boundaries (scripts/setup-repo-gh-cli.sh initializes labels)
