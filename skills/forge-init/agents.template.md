...
Load `Skill(skill='forge')` before every user request. Before reasoning. Before answering. Before any tool call. This is mandatory. When any prompt — mine, yours, a skill's, this file's — uses `mandatory`, `MUST`, `must`, `always`, follow it exactly. Before any other instruction. No hedge.
