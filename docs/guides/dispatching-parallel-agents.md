...
- Any task `Skill(skill='forge')` hands off to a phase (map fog areas, deep-research passes, SDD per-task swarm, multi-branch conflict resolution)
...
3. **Write the prompt template** — One `prompt_template` containing `{{item}}`; every filled-in prompt must be distinct. Include a **MANDATORY FIRST** block naming required skills (e.g. `Skill(skill='using-git-worktrees')`, `Skill(skill='conventional-commits')`, `Skill(skill='caveman-commit')`, `Skill(skill='verification-before-completion')`; reviewers add `Skill(skill='caveman-review')`).
...
5. **Review and integrate** — Read each summary, check for overlap, run full test suite, spot-check one claim per agent. Load `Skill(skill='verification-before-completion')` before reporting done.
...
- `Skill(skill='forge')` — Orchestrator that invokes DPA in multiple phases (map fog areas, deep-research, SDD, conflict resolution); mandates Role Mandate block in every DPA prompt template.
- `Skill(skill='subagent-driven-development')` — Uses DPA for per-task subagent swarms within worktrees; DPA handles the parallel dispatch, SDD owns worktree lifecycle and integration.
- `Skill(skill='deep-research')` — Uses DPA for parallel research passes (not `Skill(skill='loops')`); DPA is the single home for parallel subagent swarms.
- `Skill(skill='resolving-merge-conflicts')` — Delegates multi-branch conflicts to SDD, which uses DPA to unblock in parallel.
- `Skill(skill='verification-before-completion')` — Loaded after every DPA swarm to verify agent claims against actual diffs and test runs.
- `Skill(skill='caveman-commit')` / `Skill(skill='conventional-commits')` — Required in MANDATORY FIRST for implementer agents.
- `Skill(skill='caveman-review')` — Required in MANDATORY FIRST for reviewer agents.
- `Skill(skill='using-git-worktrees')` — Required in MANDATORY FIRST for worktree-based parallel work.
...
- `Skill(skill='loops')` is explicitly *not* used for DPA work (deep-research refinement passes use DPA, not loops).