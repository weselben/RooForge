...
2. **Map** — Load existing wayfinder map (by URL/number) or load `Skill(skill='wayfinder')` **chart** mode (grilling + domain-modeling → map + tickets). Parallel fog resolution via `Skill(skill='dispatching-parallel-agents')` when multiple independent fog patches exist.
...
4. **Plan** — When frontier empty, load `Skill(skill='planning-and-task-breakdown')`, enter plan mode, write plan file, request user approval. **Do not exit plan mode yourself.**
