...
Run the full suite. If tests fail, report failures and stop — everything below comes after a green suite. Load `Skill(skill='verification-before-completion')` — "tests passed earlier" is not evidence.
...
**If merge conflicts occur during merge**, load `Skill(skill='resolving-merge-conflicts')` — it handles steps 1–3, and for multi-branch conflicts delegates to `Skill(skill='subagent-driven-development')` to unblock in parallel.
