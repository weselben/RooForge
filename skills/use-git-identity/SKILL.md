...
> **Note:** the defaults below are the maintainer's identity. `Skill(skill='forge-setup')` step 6 prompts the user on first install and rewrites these values with the user's own `user.name` and `user.email` before any commit lands. Until that runs, commits carry the maintainer's identity.
