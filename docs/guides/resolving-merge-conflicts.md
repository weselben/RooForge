...
Resolves in-progress git merge/rebase conflicts hunk-by-hunk, preserving both change intents where possible. When conflicts span multiple subagent branches being integrated into the integration branch, delegates remaining hunks to `Skill(skill='subagent-driven-development')` for parallel per-file fix worktrees, then verifies the integrated result.
...
- `Skill(skill='forge')` or `Skill(skill='subagent-driven-development')` integration step reports merge conflicts
- PR push fails due to merge conflicts during `Skill(skill='pr-resolve')` loop
...
- `skills/resolving-merge-conflicts/SKILL.md` — Main skill definition: 5-step core resolution flow plus escalation to `Skill(skill='subagent-driven-development')` for multi-branch conflicts.

## See also

- `Skill(skill='subagent-driven-development')` — Orchestrates per-task worktrees and parallel fix subagents when conflicts span multiple integration branches; invoked by this skill's escalation path.
- `Skill(skill='forge')` — Session orchestrator that loads this skill during integration (step 4 work phase) and PR resolution (step 8); references conflict handling in its flow description (`skills/forge/SKILL.md:58-62`, `skills/forge/SKILL.md:124-127`).
- `Skill(skill='verification-before-completion')` — Runs full test suite on merged result after conflict resolution; required by this skill's step 7 and forge's verify phase.
- `Skill(skill='dispatching-parallel-agents')` — Underlying parallel subagent mechanism used by `Skill(skill='subagent-driven-development')` for multi-branch conflict unblocking.
- `Skill(skill='using-git-worktrees')` — Worktree management for isolated fix branches during parallel conflict resolution.

## Notes

- The skill directory contains only `SKILL.md` — no companion scripts, templates, or additional files.
- The escalation to `Skill(skill='subagent-driven-development')` is described at a protocol level; no concrete invocation pattern (command, arguments, or subagent prompt template) is specified in the source.
- Forge's reference to this skill in step 8 (`Skill(skill='pr-resolve')` loop) notes it "runs steps 1–3" but the skill itself has 5 core steps plus escalation — possible ambiguity on which steps are meant.