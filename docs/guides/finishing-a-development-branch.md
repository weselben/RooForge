...
1. **Commit all work** — Stage everything; use `caveman-commit` + `conventional-commits` (skills/caveman-commit/, skills/conventional-commits/)
...
1. **Verify tests** — Run full suite on the tree to integrate. Load `verification-before-completion`. "Tests passed earlier" is not evidence.
...
4. **Merge each subagent branch** — For each branch a subagent reported (cross-reference `subagent-driven-development`):
   ```bash
   ...
   If merge conflicts occur, load `resolving-merge-conflicts` — handles steps 1–3, delegates multi-branch conflicts to `subagent-driven-development`.

5. **Push and create PR** — `git push -u origin <integration-branch>` (detached: `git push origin HEAD:refs/heads/<new>`), follow `creating-pull-requests`. Keep worktrees for PR feedback.
...
- `caveman-commit` — Used in Subagent Variant step 1 for committing
- `conventional-commits` — Used in Subagent Variant step 1 for commit formatting
- `verification-before-completion` — Loaded in Coordinator Variant step 1 and forge step 6
- `creating-pull-requests` — Used in Coordinator Variant step 5 for PR creation
- `subagent-driven-development` — Cross-referenced for subagent branch reporting and multi-branch conflict delegation
- `resolving-merge-conflicts` — Loaded when merge conflicts occur during integration
- `forge` — Orchestrator that uses this skill in its PR/verify/review/resolve loop (steps 5–8)
